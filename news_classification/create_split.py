...
def convert_dataset_to_jsonl(cfg):
    ...
    train, validate, test = np.split(df.sample(frac=1, random_state=123),
                                     [int(0.7 * len(df)),
                                      int((0.7 + 0.15) * len(df))])
    train.to_json(os.path.join(cfg.preprocessed_dataset_path, 'train.jsonl'), force_ascii=False, lines=True,
                  orient='records')
    validate.to_json(os.path.join(cfg.preprocessed_dataset_path, 'valid.jsonl'), force_ascii=False, lines=True,
                     orient='records')
    test.to_json(os.path.join(cfg.preprocessed_dataset_path, 'test.jsonl'), force_ascii=False, lines=True,
                 orient='records')


def load_train_valid_test(cfg):
